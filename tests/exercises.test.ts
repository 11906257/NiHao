...
import {normalizePinyin,checkAnswer,wordExercise,grammarExercise,taskExercise} from '../src/lib/exercises';
...
describe('Aktives Erinnern',()=>{
 ...
 it('macht jede kommunikative Teilkompetenz übbar',()=>{for(const t of tasks)expect(taskExercise(t).practiceId).toBe(t.id);});
});
...
it('akzeptiert neutrale Töne, kombinierte Unicode-Zeichen und alle ü-Tonzeichen',()=>{
 ...
 for(const [index,mark] of [...'ǖǘǚǜ'].entries())expect(normalizePinyin(`lü${index+1}`)).toBe(`l${mark}`);
...
it('prüft deutsche Bedeutungen mit begrenzten Schreibvarianten statt Selbstkorrektur',()=>{
 const e=wordExercise(vocabulary.find(w=>w.hanzi==='早饭')!,'meaning',vocabulary);
 for(const answer of ['Frühstück','das Frühstück','Fruehstueck!'])expect(checkAnswer(e,answer)).toBe(true);
 for(const answer of ['Abendessen','kein Frühstück','Mittagessen'])expect(checkAnswer(e,answer)).toBe(false);
});
it('entfernt alte Zeichen- und Tonübungen ohne andere Fortschritte zu verlieren',()=>{
 let p=recordPractice(createProfile(),'h001',true);p=recordPractice(p,'tone-2',true);p=recordPractice(p,grammar[0].id,true);
 const restored=parseBackup(exportBackup(p),validIds,validLessonIds,validPracticeIds);
 expect(Object.keys(restored.practice)).toEqual([grammar[0].id]);expect(p.practice).toHaveProperty('h001');
});

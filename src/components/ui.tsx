...
export function Modal({title,children,onClose,className=''}:{title:string;children:ReactNode;onClose:()=>void;className?:string}){
 ...
 return <dialog ref={dialog} className={`modal ${className}`} onCancel={e=>{e.preventDefault();onClose();}} onClick={e=>{if(e.target===dialog.current)onClose();}}><div className="modal-top"><h2>{title}</h2><button className="icon-button" onClick={onClose} aria-label="Schließen"><X/></button></div>{children}</dialog>;
}
...
export function LookupDetail({title,hanzi,pinyin,meaning,note,onClose,action,children}:{title:string;hanzi:string;pinyin:string;meaning:string;note?:string;onClose:()=>void;action?:ReactNode;children:ReactNode}){
 return <Modal title={title} onClose={onClose} className="lookup-modal"><div className="lookup-detail"><header className="lookup-heading"><span className="chinese detail-hanzi" lang="zh-CN">{hanzi}</span><span className="intro-pinyin">{pinyin}</span><h2>{meaning}</h2><div className="lookup-actions"><AudioButton text={hanzi} label="Anhören"/>{action}</div></header>{note&&<p className="note">{note}</p>}<div className="lookup-content">{children}</div></div></Modal>;
}
